...
exports.normalizeLayoutJson = normalizeLayoutJson;
exports.createLayoutFromSeats = createLayoutFromSeats;
exports.saveCarriageLayoutAdmin = saveCarriageLayoutAdmin;
exports.syncCarriageSeatsAdmin = syncCarriageSeatsAdmin;
exports.bulkSyncCarriageSeatsAdmin = bulkSyncCarriageSeatsAdmin;
exports.duplicateCarriageAdmin = duplicateCarriageAdmin;
...
function seatNumberFromPosition(row, col) {
...
function seatPositionFromCode(code) {
...
function dedupeSeatInputs(seats) {
...
function inferOneBasedPositions(rawSeats) {
...
function normalizeAxisIndex(value, fallback, oneBased, maxExclusive) {
    const parsed = typeof value === 'string' ? Number(value) : Number(value);
...
function buildLayoutCells(rows, cols, seats) {
...
function normalizeLayoutJson(value, fallbackRows = 8, fallbackCols = 4) {
    if (!isRecord(value)) {
        return {
            rows: fallbackRows,
            cols: fallbackCols,
...
function createLayoutFromSeats(seats) {
...
function getNextCarriageCode(existingCodes, baseCode) {
...
async function saveCarriageLayoutAdmin(carriageId, layout) {
    const carriage = await prisma_1.prisma.carriage.findUnique({ where: { id: carriageId } });
    if (!carriage) {
...
async function syncCarriageSeatsAdmin(carriageId, layout) {
    ...
    if (blockedSeat) {
        throw new errors_1.AppError('Không thể đồng bộ ghế vì toa đã có đặt chỗ. Hãy nhân bản toa trước khi chỉnh layout.', 409);
    }
    const normalized = normalizeLayoutJson(layout);
    const uniqueSeats = dedupeSeatInputs(normalized.seats.map((seat) => ({
...
async function bulkSyncCarriageSeatsAdmin(carriageId, seats, layout) {
    const carriage = await prisma_1.prisma.carriage.findUnique({
        where: { id: carriageId },
        include: {
            train: true,
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (normalizedSeats.length > 0) {
            await tx.seat.createMany({
                data: normalizedSeats.map((seat) => ({
                    ...
                    code: seat.code,
                    status: 'ACTIVE',
                    price: seat.price === undefined || seat.price === null ? null : new decimal_js_1.default(seat.price)
                })),
                skipDuplicates: true
            });
        }
        return tx.carriage.update({
            where: { id: carriageId },
            data: layout === undefined ? {} : { layout: normalizeLayoutJson(layout) },
            include: {
                train: true,
                seats: {
                    orderBy: {
...
async function duplicateCarriageAdmin(carriageId, newCode) {
    const carriage = await prisma_1.prisma.carriage.findUnique({
        where: { id: carriageId },
        include: {
            train: true,
    ...
    const existingCodes = await prisma_1.prisma.carriage.findMany({
        where: {
    ...
    const nextOrderIndex = (await prisma_1.prisma.carriage.aggregate({
        where: {
            trainId: carriage.trainId
        },
        _max: {
    ...
    return prisma_1.prisma.$transaction(async (tx) => {
        ...
        if (layout.seats.length > 0) {
            const uniqueSeats = dedupeSeatInputs(layout.seats.map((seat) => ({
                code: seat.seatNumber,
                price: seat.price ?? null
            })));
            await tx.seat.createMany({
